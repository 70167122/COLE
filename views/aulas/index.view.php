 ...
 <div id="Modal-registrar" class="modal fade"  tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="form-nuevo" action="<?php echo RUTA ?>admin/aulas/guardar.php" method="post">

            
                <div class="form-group">
                <label >Descripción</label>
                <input type="text" class="form-control1 ng-invalid ng-invalid-required ng-touched" name="Descripcion" ng-model="model.name">
              </div>
  ...
  $('.aceptar_edicion').on( "click", function() {
     ...
     <script>
      $("#bus").keyup(function(){
        ler = $("#bus").val();
        dato = {palabra : ler};
        ruta = "<?php echo RUTA ?>ajax/buscara.php";
        $.ajax({
          data : dato,
          url : ruta,
          type : 'post',
